...
void greedyScheduling(Ad ads[], int n) {
    sortByProfit(ads, n); // Sort by profit descending
    int maxDeadline = findMaxDeadline(ads, n);
    int slot[maxDeadline+1];
    for (int i = 0; i <= maxDeadline; i++) slot[i] = -1;

    int totalProfit = 0;
    printf("\nSelected Ads (Greedy):\nAdID\tProfit\tDeadline\tScheduledSlot\n");
    for (int i = 0; i < n; i++) {
        for (int j = ads[i].deadline; j > 0; j--) {
            if (slot[j] == -1) { // allocate latest free slot before deadline
                slot[j] = ads[i].id;
                totalProfit += ads[i].profit;
                printf("%d\t%d\t%d\t\t%d\n", ads[i].id, ads[i].profit, ads[i].deadline, j);
                break;
            }
        }
    }
...
// Dynamic Programming approach for scheduling ads with slot tracking
void dpscheduling(Ad ads[], int n) {
    int maxDeadline = findMaxDeadline(ads, n);

    // dp[i][j] = max profit using first i ads with j slots available
    ...
    printf("\nMaximum Profit (DP): %d\n", dp[n][maxDeadline]);
    printf("Selected Ads (DP) with Scheduled Slots:\nAdID\tProfit\tDeadline\tScheduledSlot\n");

    // Backtrack to find selected ads and slots
    int res = dp[n][maxDeadline];
    int j = maxDeadline;
    int scheduledSlots[maxDeadline + 1];
    for (int i = 0; i <= maxDeadline; i++) scheduledSlots[i] = -1; // init slots

    for (int i = n; i > 0 && res > 0; i--) {
        if (res != dp[i - 1][j]) {
            // Assign ad to slot j (latest available before deadline)
            scheduledSlots[j] = ads[i - 1].id;
            printf("%d\t%d\t%d\t\t%d\n", ads[i - 1].id, ads[i - 1].profit, ads[i - 1].deadline, j);
            res -= ads[i - 1].profit;
            j--;
        }
    }
